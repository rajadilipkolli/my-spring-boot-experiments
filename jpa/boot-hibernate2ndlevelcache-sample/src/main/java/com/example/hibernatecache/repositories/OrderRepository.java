...
public interface OrderRepository extends BaseJpaRepository<Order, Long>, PagingAndSortingRepository<Order, Long> {
    ...
    @Query("SELECT o FROM Order o left join fetch o.orderItems oi WHERE o.id = :id ORDER BY oi.itemCode")
    Optional<Order> findById(@Param("id") Long id);

    @QueryHints(@QueryHint(name = HINT_CACHEABLE, value = "true"))
    @Query(
            "SELECT o FROM Order o left join fetch o.orderItems oi WHERE o.customer.id = :customerId ORDER BY oi.itemCode")
