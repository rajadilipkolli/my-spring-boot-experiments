...
class StrategyPluginApplicationTests extends AbstractIntegrationTest {
  ...
  private static final Logger logger = LoggerFactory.getLogger(StrategyPluginApplicationTests.class);
    ...
    this.mockMvcTester.get().uri("/fetch").param("type", "xls")
            ...
            .satisfies(problemDetail -> {
                ...
                assertThat(problemDetail.getInstance()).isNotNull()
                    .extracting(Object::toString).isEqualTo("/fetch");
            });
  }
  ...
  void verifyLogsAreWrittenToLokiWithCorrectFormat() {
    ...
    System.out.println("✓ Loki4j 2.0 migration verified: logs are being sent to Loki successfully");
  }

  private void verifyLogsInLoki() {
    RestClient lokiClient = RestClient.builder()
            .baseUrl("http://localhost:3100")
            .build();

    // Check if Loki is available and has labels
    JsonNode labelsResponse = lokiClient.get()
    ...
    for (String query : queries) {
        try {
            response = lokiClient.get()
                    .uri("/loki/api/v1/query_range?query={query}&start={start}&end={end}&limit=100", 
                         query, oneHourAgo, now)
                    .retrieve()
                    .body(JsonNode.class);
    ...
    assertThat(response).isNotNull()
            .withFailMessage("Could not find any logs in Loki");
    ...
    assertThat(foundTestLog || entries.size() > 0).isTrue()
