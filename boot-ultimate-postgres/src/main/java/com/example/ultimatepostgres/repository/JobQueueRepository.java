...
public interface JobQueueRepository extends JpaRepository<JobQueueEntity, Long> {
    ...
    @Query(
            "UPDATE JobQueueEntity j SET j.status = com.example.ultimatepostgres.model.JobStatus.IN_PROGRESS, j.updatedAt = CURRENT_TIMESTAMP WHERE j.id IN :ids")
    int markInProgress(@Param("ids") List<Long> ids);

    @Modifying
    @Query(
            "UPDATE JobQueueEntity j SET j.status = com.example.ultimatepostgres.model.JobStatus.COMPLETED, j.updatedAt = CURRENT_TIMESTAMP WHERE j.id = :id")
    int markCompleted(@Param("id") Long id);

    @Modifying
    @Query(
            "UPDATE JobQueueEntity j SET j.status = com.example.ultimatepostgres.model.JobStatus.FAILED, j.updatedAt = CURRENT_TIMESTAMP WHERE j.id = :id")
    int markFailed(@Param("id") Long id);

    @Modifying
    @Query(
            "UPDATE JobQueueEntity j SET j.status = com.example.ultimatepostgres.model.JobStatus.PENDING, j.attemptCount = j.attemptCount + 1, "
                    + "j.availableAt = :retryAt, j.updatedAt = CURRENT_TIMESTAMP WHERE j.id = :id AND j.status = com.example.ultimatepostgres.model.JobStatus.FAILED")
